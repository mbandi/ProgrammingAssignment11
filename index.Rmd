  ...
  server = function(input, output) {
    output$map <- renderLeaflet({
        my_map <- leaflet() %>% 
            ...
            addMarkers(lat = 39.565480, lng = 2.653512, layerId = "bellaombra", 
                       popup = "Bellaombra", group = "Landmarks") %>%
            addMarkers(lat = 39.570967, lng = 2.640377, layerId = "saFeixina", 
                       popup = "Big park", group = "Landmarks") %>%
            addMarkers(lat = 39.567634, lng = 2.647568, layerId = "laSeu", 
                       popup = "Cathedral", group = "Landmarks") %>%
            addMarkers(lat = 39.571418, lng = 2.649283, layerId = "plMercat", 
                       popup = "Cool modernist buildings", group = "Landmarks") %>%
            addMarkers(lat = 39.569623, lng = 2.649969, layerId = "cort", 
                       popup = "Old olive tree", group = "Landmarks") %>%
            addMarkers(lat = 39.575540, lng = 2.653417, layerId = "plEspanya", 
                       popup = "Plaza España", group = "Landmarks") %>%

            
            addMarkers(lat = 39.571252, lng = 2.653434, layerId = "claudio", 
                       popup = "Best Ice cream", group = "Food/drink") %>%
            addMarkers(lat = 39.567758, lng = 2.654576, layerId = "templeNatura", 
                       popup = "A vegan restaurant", group = "Food/drink") %>%
            addMarkers(lat = 39.583628, lng = 2.655691, layerId = "quemado", 
                       popup = "Great pizza", group = "Food/drink") %>%
            addMarkers(lat = 39.572195, lng = 2.639348, layerId = "cFabrica", 
                       popup = "Bars and restaurants", group = "Food/drink") %>%
            addMarkers(lat = 39.578447, lng = 2.650977, layerId = "cBlanquerna", 
                       popup = "Coffe and restaurants", group = "Food/drink") %>%
            addMarkers(lat = 39.579886, lng = 2.650021, layerId = "maua", 
                       popup = "Artisanal chocolate store", group = "Food/drink") %>%

            
            addMarkers(lat = 39.565157, lng = 2.655702, layerId = "baluardPrincep", 
                       popup = "'Restored' old wall", group = "Nice Views") %>%
            addMarkers(lat = 39.571935, lng = 2.651175, layerId = "rambla", 
                       popup = "Stairs with some views", group = "Nice Views") %>%
            addMarkers(lat = 39.566989, lng = 2.647692, layerId = "dMurada", 
                       popup = "Old wall", group = "Nice Views") %>%
            addMarkers(lat = 39.569665, lng = 2.641128, layerId = "baluard1", 
                       popup = "To see the bay", group = "Nice Views") %>%
            addMarkers(lat = 39.570545, lng = 2.641775, layerId = "baluard2", 
                       popup = "To see the old town roofs", group = "Nice Views") %>%
            addMarkers(lat = 39.563842, lng = 2.619530, layerId = "bellver", 
                       popup = "To see the city", group = "Nice Views") %>%

            addMarkers(lat = 39.566282, lng = 2.650319, layerId = "colArq", 
                       popup = "Architecture exposition", group = "To visit") %>%
            addMarkers(lat = 39.567179, lng = 2.650736, layerId = "museoMall", 
                       popup = "Historic Art exposition", group = "To visit") %>%
            addMarkers(lat = 39.570140, lng = 2.641042, layerId = "musBaluard", 
                       popup = "Modern art", group = "To visit") %>%
            addMarkers(lat = 39.570744, lng = 2.648075, layerId = "puzzle2", 
                       popup = "Cool mind teasers", group = "To visit") %>%
    ...
    observeEvent(input$map_marker_click, {
        ...
        if(markerId == "bellaombra") {
            output$description <- renderText({"We used to have beautiful trees here, until the city council decided to chop them down :("})
